        ...
        <div class="side-content">
          <div class="side-ppt-list scroll">
            ...
            <div
              ...
              :data-id="item.id"
              :class="[
                { draging: isEleDraging },
                { listCompleteItem: isShowAnimate },
  ...
  methods: {
    ...
    elementPanStart(ev) {
      ...
      if ($box) {
        ...
        this.dragingTag.indexItem = this.countList[_index];
      ...
      if (_self.isEleDraging) {
        ...
        if ($container) {
          ...
          if (_id != this.dragingTag.dragId && hoverIndex != "null") {
            ...
            _countList[endIndex] = this.dragingTag.indexItem;
            this.countList = _countList;
          }
        }
      }
      this.sortCanvasArr(this.countList);
    },
    ...
    /** 缩略图移动之后对 canvasArr 进行对应的排序（预览） */
    sortCanvasArr(orign) {
      console.log(this.canvasInfo.canvasArr);
      const result = this.canvasInfo.canvasArr.sort(function(a, b) {
        return orign.map(s => s.id).indexOf(a.id) <
          orign.map(s => s.id).indexOf(b.id)
          ? -1
          : 1;
      });
      this.canvasInfo.canvasArr = result;
    },
    /**添加空白幻灯片 */
    addCanvas() {
    ...
    saveImgInfo(canvasImg) {
      ...
      if (this.canvasInfo.canvasThum.length > 0) {
        ...
        for (var i = 0; i < this.canvasInfo.canvasThum.length; i++) {
          if (this.canvasInfo.canvasThum[i].t_id == this.imgId) {
            this.canvasInfo.canvasThum[i].img = canvasImg;
          }
        }
        // 没有保存过.t_id 为 imgId 的画布
        if (this.tIdArr.indexOf(this.imgId) == -1) {
          imgItem.t_id = this.imgId;
    ...
    getThumImg(id) {
      if (this.canvasInfo.canvasThum.length > 0) {
        for (var i = 0; i < this.canvasInfo.canvasThum.length; i++) {
          if (this.canvasInfo.canvasThum[i].t_id == id) {
    ...
    delCanvas(index) {
      if (this.countList.length == 1) {
        this.$utils.tipBox("已经是最后一张啦！", "error");
        return false;
      }
      ...
      for (var j = 0; j < this.canvasInfo.canvasArr.length; j++) {
        if (this.canvasInfo.canvasArr[j].id == this.imgId) {
